...
=== Basic Objects
  ...
  me = User.new('me', :token => YOUR_ACCESS_TOKEN)
...
=== Connections
  ...
  # Private connections requires "token"
  FbGraph::User.new('matake').friends # => raise FbGraph::Unauthorized
  user = FbGraph::User.fetch('matake', :token => ACCESS_TOKEN)
  user.albums
  user.events
  user.friends
  user.likes
  ...
  me = User.new('me', :token => ACCESS_TOKEN)
  me.home
  :

=== Pagination

  user = FbGraph::User.new('matake', :token => ACCESS_TOKEN)
